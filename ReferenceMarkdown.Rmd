  ...
  if (!(is.numeric(X) || class(X)=="Date")){
    ...
    print(kable(sort(proportions, decreasing=TRUE), col.names = c(nom, "Proportion"), caption=paste(nom, " relative frequency table of proportions (sorted)")))
     ...
     if(class(X)=="Date"){
       ...
       tab <- matrix(c(summ[1], summ[2], summ[3], summ[4], summ[5], summ[6], sd(X, na.rm=TRUE), sd(X, na.rm=TRUE)/mean(X, na.rm=TRUE)), ncol=8, byrow=TRUE)
       colnames(tab) <- c('Min.','1st Qu.','Median', 'Mean', '3rd Qu.', 'Max.', 'sd', 'vc.')
       rownames(tab) <- c('')
       tab <- as.table(tab)
